...
async def load_items(items: list[dict], session: AsyncSession) -> int:
    ...
    for item in items:
        ...
        # Map short ID (e.g. "lab-01") to the record
...
async def load_logs(
    ...
    for item in items_catalog:
        lab_key = item["lab"]
        task_key = item.get("task")  # None for labs
        title = item["title"]
        title_lookup[(lab_key, task_key)] = title
    ...
    for log in logs:
        ...
        lab_key = log["lab"]
        task_key = log.get("task")  # None for labs
        item_title = title_lookup.get((lab_key, task_key))

        if item_title is None:
            continue  # Skip log if no matching item found
        ...
        # Step 3: Check if InteractionLog already exists (idempotency)
